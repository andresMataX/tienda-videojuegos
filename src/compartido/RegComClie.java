...
public class RegComClie extends JFrame
{
	...
	public RegComClie()
	{
		...
		JLabel lbl_tit = new JLabel("REGISTRAR COMPRA - CLIENTE");
		lbl_tit.setFont(new Font("Roboto", Font.PLAIN, 38));
		lbl_tit.setBounds(113, 75, 540, 45);
		...
		JLabel lbl_cnd_com = new JLabel("CONDICI\u00D3N DEL ART\u00CDCULO");
		lbl_cnd_com.setFont(new Font("Roboto", Font.PLAIN, 24));
		lbl_cnd_com.setBounds(23, 323, 300, 28);
		getContentPane().add(lbl_cnd_com);
		...
		@SuppressWarnings("rawtypes")
		JComboBox cbox_cnd_com = new JComboBox();
		cbox_cnd_com.setForeground(Color.BLACK);
		cbox_cnd_com.setFont(new Font("Roboto", Font.PLAIN, 24));
		cbox_cnd_com.setBorder(new LineBorder(new Color(0, 0, 0)));
		cbox_cnd_com.setBackground(Color.WHITE);
		cbox_cnd_com.setBounds(21, 351, 350, 28);
		getContentPane().add(cbox_cnd_com);
		
		CbRellenar sc_cnd = new CbRellenar();
		@SuppressWarnings("rawtypes")
		DefaultComboBoxModel mCondicion = new DefaultComboBoxModel(
				sc_cnd.mostrarCbox("select cve_cnd, nom_cnd from condicion", "Seleccionar Condición", "cve_cnd", "nom_cnd"));
		cbox_cnd_com.setModel(mCondicion);
		...
		btn_aceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				CbRellenar index_cnd = (CbRellenar) cbox_cnd_com.getSelectedItem();
				if (!new Funcionalidades().validadorAcep(txt_nom_com, txt_pre_com, txt_dia_com, txt_mes_com, txt_anio_com, index_cat.getId(), index_cnd.getId()))
				{		
					SentenciasSQL c = new SentenciasSQL();					
					String statement_com = "insert into compras values (?,?,?,?,?)";
					String[] reg_com = {"0", Login.cve_user+"", txt_pre_com.getText(), "1", deco.fecha()};
					int[] tipo_com = {1,1,3,1,2};
					c.createSQL(statement_com, reg_com, tipo_com);					

					
					String statement_art = "insert into articulos values (?,?,?,?,?,?,?)";
					String[] reg_art = {"0", txt_nom_com.getText(), txt_pre_com.getText(), "1", index_cat.getId()+"", index_cnd.getId()+"","1"};
