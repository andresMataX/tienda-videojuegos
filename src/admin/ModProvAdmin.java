...
public class ModProvAdmin extends JFrame
{
	public ModProvAdmin()
	{
		...
		JLabel lbl_tit = new JLabel("MODIFICAR PROVEEDOR");
		...
		btn_aceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(!new Funcionalidades().validadorAcep(txt_nom_prov))
				{
					...
					String statement_com = "update proveedores set nom_prov = ? where cve_prov = '"+ SelecModProvAdmin.cve_prov +"'";
					String[] reg_com = {txt_nom_prov.getText()};
					int[] tipo_com = {2};
		...
		btn_reg.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new SelecModProvAdmin().setVisible(true);
				dispose();
			}
		});
		...
		try 
		{
			...
			String sql = "select nom_prov from proveedores where cve_prov = " + SelecModProvAdmin.cve_prov;
			PreparedStatement pst = cn.prepareStatement(sql);
			ResultSet rs = pst.executeQuery();
			
			while(rs.next()) 
			{
				txt_nom_prov.setText(rs.getString("nom_prov"));
			}
			cn.close();
		} 
