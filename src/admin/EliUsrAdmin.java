...
public class EliUsrAdmin extends JFrame
{
	public EliUsrAdmin()
	{
		...
		tb_usr.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				int cve_usr = new Funcionalidades().selecTabla(model, 0, tb_usr, e);
				int seleccion = JOptionPane.showConfirmDialog(null, "¿Estás seguro de eliminar " + cve_usr + "?", "Confirmar Eliminar Artículo", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE);
				if (seleccion == 0)
				{
					SentenciasSQL eli = new SentenciasSQL();
					eli.deleteSQL("delete from usuarios where cve_usr = '" + cve_usr + "'");
					model.setRowCount(0);
					mostrarTabla(model, tb_usr);
				}
			}
		});
		...
		JLabel lbl_tit = new JLabel("ELIMINAR - USUARIO");
		lbl_tit.setBounds(203, 75, 360, 45);
