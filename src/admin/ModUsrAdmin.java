...
public class ModUsrAdmin extends JFrame
{
	public ModUsrAdmin()
	{
		...
		JLabel lbl_tit = new JLabel("MODIFICAR USUARIO");
		lbl_tit.setFont(new Font("Roboto", Font.PLAIN, 38));
		lbl_tit.setBounds(203, 75, 370, 45);
		...
		btn_aceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!new Funcionalidades().validadorAcep(txt_app_usr, txt_apm_usr, txt_call_usr, txt_tel_usr, txt_nom_usr, txt_col_usr))
				{
					SentenciasSQL c = new SentenciasSQL();					
					String statement_com = "update usuarios set nom_usr = ?, app_usr = ?, apm_usr = ?, call_usr = ?,col_usr = ?,tel_usr = ? where cve_usr = '"+ SelecModUsrAdmin.cve_usr +"'";
					String[] reg_com = {txt_nom_usr.getText(), txt_apm_usr.getText(), txt_app_usr.getText(), txt_call_usr.getText(), txt_col_usr.getText(), txt_tel_usr.getText()};
					int[] tipo_com = {2,2,2,2,2,2};
		...
		btn_regresar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new SelecModUsrAdmin().setVisible(true);
				dispose();
			}
		});
		...
		try 
		{
			...
			String sql = "select apm_usr, app_usr,call_usr,col_usr,nom_usr,tel_usr  from usuarios where cve_usr = " + SelecModUsrAdmin.cve_usr;
